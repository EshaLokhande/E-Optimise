...
function activate(context) {
    ...
    let historyDisposable = vscode.commands.registerCommand('e-optimise.viewHistory', async () => {
        try {
            const records = await (0, analyzer_1.fetchAnalysisHistory)(20);
...
async function handleAnalysis(code, language, extra) {
    ...
    try {
        if (choice.label === '$(lightbulb) Visualise Function') {
            const result = await (0, analyzer_1.generateDiagram)(code, language);
        ...
        if (choice.label === '$(symbol-ruler) Get Big-O Notation') {
            const result = await (0, analyzer_1.analyzeCode)(code, language);
            const analysis = [
        ...
        if (choice.label === '$(tools) Optimise Function') {
            const result = await (0, analyzer_1.optimizeCode)(code, language);
            const analysis = [
                `\ud83d\udca1 Improvements:`,
                ...result.suggestions.map(s => `  \u2022 ${s}`),
                ``,
...
function showHistoryPanel(records) {
    const panel = vscode.window.createWebviewPanel('e-optimise-history', 'E-Optimise: Analysis History', vscode.ViewColumn.Beside, { enableScripts: true });
...
function showPanel(original, analysis, mermaidCode, title, extra) {
...
function getWebviewContent(code, analysis, mermaidCode, title, extra) {
